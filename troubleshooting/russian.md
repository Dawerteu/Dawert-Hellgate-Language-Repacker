...
- Если `Launcher.exe` не работает с измененными данными или снова скачивает оригинальную локализацию, для игры используйте `play-windows.bat`.
...
- Если launcher перезаписывает переведенные файлы, запускайте игру через:
...
Скрипт `play-*` имеет режим обычной игры и режим официального обновления. Режим обновления сначала восстанавливает чистые данные, запускает `Launcher.exe`, после закрытия обновляет чистый backup и может снова применить перевод.
